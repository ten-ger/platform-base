...
export class IntegerInput {
  ...
  */
  ...
  async setValue(value: number) {
    ...
    let elem = await this.inputElem.getInputElement();
    elem.value = this.fieldValue.toString();
  }
  ...
  async validate() {
    ...
    else if (this.fieldValue && this.fieldValue.toString().includes('.')) {
      this.validationError = 'Value cannot contain decimal.'
    }
